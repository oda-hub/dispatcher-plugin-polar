...
class DummyPolarRes(object):
    ...
    def json(self):
        ...
        data = NumpyDataProduct.from_fits_file(getattr(self, "dummy_lc"))
        ...
        _d["root_file_b64"] = getattr(self, "dummy_root")
        return _d
...
class PolarLigthtCurve(LightCurveProduct):
    ...
    def build_from_res(
        ...
        if res_json["status"]["success"]:
            data = np.zeros(
                len(df["rate"]),
                dtype=[("rate", "<f8"), ("rate_err", "<f8"), ("time", "<f8")],
            )
            data["rate"] = df["rate"]
            data["rate_err"] = df["rate_err"]
            data["time"] = df["time"]
            npd = NumpyDataProduct(
                data_unit=NumpyDataUnit(
                    data=data, hdu_type="bintable", name="POLAR_LC"
                ),
                meta_data=meta_data,
            )
            ...
            raise PolarAnalysisException(
                message="polar light curve failed: %s" % _d, debug_message=_d["kind"]
            )

        if skip_root is False:
            try:
                open(root_file_path.path, "wb").write(
                    BinaryData().decode(res_json["root_file_b64"])
                )
                ...
                print(e)
                raise PolarAnalysisException(
                    message="polar failed to open/decode root_file"
                )
        else:
...
class PolarLightCurveQuery(LightCurveQuery):
    ...
    def build_product_list(
        self, instrument, res, out_dir, prod_prefix="polar", api=False
    ):

        delta_t = instrument.get_par_by_name("time_bin")._astropy_time_delta.sec
        prod_list = PolarLigthtCurve.build_from_res(
            res,
            src_name="lc",
            prod_prefix=prod_prefix,
            out_dir=out_dir,
    ...
    def process_product_method(self, instrument, prod_list, api=False):
        ...
        for query_lc in prod_list.prod_list:
            ...
            if api == False:
                ...
                _html_fig.append(
                    query_lc.get_html_draw(
                        x=query_lc.data.data_unit[1].data["time"],
                        y=query_lc.data.data_unit[1].data["rate"],
                        dy=query_lc.data.data_unit[1].data["rate_err"],
                        title="Start Time: %s"
                        % instrument.get_par_by_name("T1")._astropy_time.utc.value,
                        x_label="Time  (s)",
                        y_label="Rate  (cts/s)",
                    )
                )
    ...
    def get_dummy_products(
        self, instrument, config, out_dir="./", prod_prefix="polar", api=False
    ):
        ...
        prod_list = PolarLigthtCurve.build_from_res(
            res, src_name="lc", prod_prefix=prod_prefix, out_dir=out_dir, skip_root=True
        )
